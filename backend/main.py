...
def _last_condition(data: AppData, session_id: str) -> Condition:
    ...
    return Condition(
        energy="MEDIUM", focus="NORMAL", location="HOME", environment="QUIET"
    )
...
def update_journey(journey_id: str, payload: Dict[str, Any]):
    ...
    append_audit_event(
        data, "JOURNEY_UPDATED", {"previous": prev, "current": journey.model_dump()}
    )
    write_app_data(data)
    return journey
...
def update_node(node_id: str, payload: Dict[str, Any]):
    ...
    for field in ("name", "description", "node_type", "done_type", "due_date", "note"):
        if field in payload:
            setattr(
                node, field, str(payload[field]).strip() if payload[field] else None
            )
    ...
    append_audit_event(
        data, "NODE_UPDATED", {"previous": prev, "current": node.model_dump()}
    )
    write_app_data(data)
    return node
...
def close_node(node_id: str, payload: Dict[str, Any]):
    ...
    if not reason:
        raise HTTPException(
            status_code=400, detail="A reason is required to close a node"
        )
    ...
    append_audit_event(
        data,
        "NODE_CLOSED",
        {"node_id": node_id, "reason": reason, "node": node.model_dump()},
    )
    write_app_data(data)
    return node
...
def set_node_estimate(node_id: str, payload: Dict[str, Any]):
    ...
    event_type = (
        "NODE_ESTIMATE_SET" if prev_estimate is None else "NODE_ESTIMATE_REVISED"
    )
    append_audit_event(
        data,
        event_type,
...
def get_sessions(
    journey_id: Optional[str] = Query(None), status: Optional[str] = Query(None)
):
    data = read_app_data()
...
def create_session(payload: Dict[str, Any]):
    ...
    if not intention:
        raise HTTPException(
            status_code=400, detail="intention is required and cannot be empty"
        )
...
def end_session(session_id: str, payload: Dict[str, Any]):
    ...
    session.reflection = (
        str(payload["reflection"]).strip() if payload.get("reflection") else None
    )
    ...
    append_audit_event(
        data,
        "SESSION_COMPLETED" if new_status == "COMPLETE" else f"SESSION_{new_status}",
        session.model_dump(),
    )
    if session.reflection:
        append_audit_event(
            data,
            "SESSION_REFLECTION_ADDED",
            {
...
def revise_intention(session_id: str, payload: Dict[str, Any]):
    ...
    if session.status != "ACTIVE":
        raise HTTPException(
            status_code=400, detail="Can only revise intention on an active session"
        )
    ...
    entry = SessionEntry(
        ...
        logged_at=now,
        note=f"Revised to: {new_intention}"
...
def switch_journey_session(session_id: str, payload: Dict[str, Any]):
    ...
    if not new_intention:
        raise HTTPException(
            status_code=400, detail="new_intention is required for the new session"
        )
    now = get_current_iso()
    condition = _condition_from_payload(payload, _last_condition(data, session_id))
...
def create_session_entry(session_id: str, payload: Dict[str, Any]):
    ...
    if session.status != "ACTIVE":
        raise HTTPException(
            status_code=400, detail="Cannot log entries on a session that is not active"
        )
    ...
    if entry_type not in valid_types:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid entry_type. Must be one of: {', '.join(sorted(valid_types))}",
        )
    now = get_current_iso()
    condition = _condition_from_payload(payload, _last_condition(data, session_id))
    ...
    if node_id:
        ...
        if node.journey_id != session.journey_id:
            raise HTTPException(
                status_code=400, detail="Node does not belong to the session's journey"
            )
    ...
    if entry_type == "DISCOVERY":
        ...
        if not discovery_payload or not str(discovery_payload.get("name", "")).strip():
            raise HTTPException(
                status_code=400,
                detail="discovery_node.name is required for DISCOVERY entries",
            )
        discovery_node = Node(
            id=f"nod-{uuid4()}",
            ...
            description=str(discovery_payload["description"]).strip()
            if discovery_payload.get("description")
            else None,
            status="PLANNED",
...
def create_correction(entry_id: str, payload: Dict[str, Any]):
    ...
    if not field or not corrected_value:
        raise HTTPException(
            status_code=400, detail="field and corrected_value are required"
        )
...
def query_journey_progress(journey_id: str = Query(...)):
    ...
    started = sum(
        status_counts.get(s, 0) for s in ("ACTIVE", "PAUSED", "DORMANT", "COMPLETE")
    )
